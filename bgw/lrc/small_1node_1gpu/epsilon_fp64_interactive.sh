...
RESULTS_DIR="${BGW_LRC}/results/EPS_SMALL_FP64_${SLURM_JOB_ID}"
...
start=$(date +%s.%N)
srun -N 1 -c 16 --ntasks-per-node=1 --gpus-per-node=1 --cpu-bind=cores ${DCGM_PATH} ./epsilon.cplx.x > ${RESULTS_DIR}/${SLURM_JOB_ID}.out
end=$(date +%s.%N)
elapsed=$(printf "%s - %s\n" $end $start | bc -l)
